...
class AFNSwiftRequest: AFHTTPSessionManager {
    static let shareInstance : AFNSwiftRequest = {
        let toolInstance = AFNSwiftRequest()
        toolInstance.responseSerializer.acceptableContentTypes? = NSSet.init(objects:"application/json", "text/json", "text/javascript","text/html","text/plain") as! Set<String>
        return toolInstance
    }()

    
    // 将成功和失败的回调写在一个逃逸闭包中
    func request(Request_Type : Request_Type, url : String, parameters : [String : Any], resultBlock : @escaping([String : Any]?, Error?) -> ()) {
        // 成功闭包
    ...
    func request(Request_Type : Request_Type, url : String, parameters : [String : Any], succeed : @escaping([String : Any]?) -> (), failure : @escaping(Error?) -> ()) {
...
extension AFNSwiftRequest {
    //MARK: - GET 请求
    //    let tools : AFNSwiftRequest.shareInstance!
...
//                当响应成功，使用临时变量value接受服务器返回的信息并判断是否为[String: AnyObject]类型 如果是那么将其传给其定义方法中的success
